...
def scrape_jumia_products(url):
    ...
    for product in products:
        ...
        try:
            ...
            discount = 'product'
        try:
            rating_tag = product.find('section', class_='col12 -df -d-co')
            if rating_tag:
                rating_stars = rating_tag.find_all('div', class_='stars _m _al')
                original_rating = len(rating_stars)
            else:
                original_rating = 0
        except AttributeError:
            original_rating = 0
        ...
        try:
            ...
            total_reviews = 0
        popularity = "High" if total_reviews >= 50 else "Low"
        product_data = {
